...
一个零依赖的双语静态博客，用于文章分享，以及 PrimePlayer、MagicDesk、Picturium 与 DeepNote 的产品介绍与开发记录。
...
3. `app` 可填 `primeplayer`、`magicdesk`、`picturium`、`deepnote`或 `general`；`category` 可填 `product`、`design`、`reflection`、`legal` 或 `support`。
...
- `content/apps.mjs`：PrimePlayer、MagicDesk、Picturium 与 DeepNote 均已配置 App Store 地址；其中的版本号是 Apple 元数据不可用时的构建兜底值。
