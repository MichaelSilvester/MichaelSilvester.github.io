...
一个零依赖的双语静态博客，用于文章分享，以及 PrimePlayer、MagicDesk、Picturium 与 Diple 的产品介绍与开发记录。
...
服务启动后会自动用默认浏览器打开本地地址；如果没有自动打开，也可以访问终端中显示的地址。保存 `content/`、`public/` 或页面生成器中的改动后，站点会自动重新构建，已打开的浏览器也会自动刷新；不需要先提交 Git。
...
3. `app` 可填 `primeplayer`、`magicdesk`、`picturium`、`diple`或 `general`；`category` 可填 `product`、`design`、`reflection`、`legal` 或 `support`。
...
- `content/apps.mjs`：PrimePlayer、MagicDesk、Picturium 与 Diple 均已配置 App Store 地址；其中的版本号是 Apple 元数据不可用时的构建兜底值。
